...
export type ConceptOption = {
  name: string; slug: string; selected: boolean; ratio: number;
  feedback: string; sketchNote?: string;
};
...
export default function ConceptCarousel({
  ...
  return (
    <div className={`ph-06-carousel${active.selected ? " ph-06-carousel-final" : ""}`}>
      ...
      <div className="ph-06-carousel-copy" aria-live="polite" aria-atomic="true">
        {active.selected ? <p className="ph-lbl ph-06-carousel-eyebrow">{selectedEyebrow}</p> : null}
        <h3>{active.name}</h3>
        {active.sketchNote ? <p className="ph-06-sketch-note">{active.sketchNote}</p> : null}
        {active.selected ? <p className="ph-06-carousel-note">{selectedNote}</p> : null}
        <p className="ph-lbl ph-06-feedback-label">Farmer feedback</p>
        <p className="ph-06-feedback">{active.feedback}</p>
