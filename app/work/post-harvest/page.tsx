  ...
  return (
    <main className={`ph-root ${geist.variable} ${geistMono.variable} ${bodoni.variable}`}>
      ...
      {/* ============ 07 The Drying Tower / Final Concept ============
          ...
          COPY. Content and specifications come from the evidence-reviewed data in
          content.ts. The background is a layout reference, not a source for claims.
      ...
      <section className="ph-fc" id="final-concept">
        <div className="ph-fc-visual">
          ...
          <Reveal className="ph-fc-ov ph-fc-ov-intro">
            ...
            <p className="ph-fc-ov-lead">{finalConcept.lead}</p>
          </Reveal>

          {/* Specifications are sourced from the report in content.ts. */}
          <Reveal className="ph-fc-ov ph-fc-ov-specs">
            <p className="ph-lbl ph-fc-ov-specs-label">The drying tower</p>
            <p className="ph-fc-ov-specs-desc">{finalConcept.body[0]}</p>
            <ul className="ph-annots ph-fc-ov-specs-annots">
              {finalConcept.specs.map((a) => (
                <li className="ph-annot" key={a.label}>
                  <span className="v">{a.value}</span>
                  <span className="k">{a.label}</span>
                  <span className="ph-spec-evidence">{a.note}</span>
                </li>
              ))}
            </ul>
          </Reveal>
      ...
      {/* ============ 09 What we completed, and what remained open ============
          The completed band documents the collector and handbook. A semantic table below
          records the evidence and limitations for each of the eighteen requirements. */}
