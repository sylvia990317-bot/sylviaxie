...
export const field = {
  ...
  captions: {
    ...
    team: "Sitting with farmers. Informal first visits helped shape a structured interview guide for later visits.",
  },
  quote: {
    text:
...
export const participants: {
...
}[] = [
  {
    ...
    // This individual account suggests a role for use practices, not a causal finding.
    name: "Theresa",
    slug: "theresa",
...
export const focus = {
  ...
  body: "Interviews revealed misunderstandings about using the bags, including a belief that they lost effectiveness after a season or two. Theresa followed the instructions and reported no weevil problem. Her experience suggested that use practices could matter; it did not establish the cause of everyone else's losses.",
  captions: {
    ...
    cycle: "Before storage, maize dries outdoors, exposed to animals, theft and rain. Farmers also described weevils in the grain.",
  },
  /**
  ...
  redirect: {
    label: "The turn",
    text: "These risks gave us a reason to move upstream: improve the drying process before maize goes into storage. The bag interviews raised a separate need for clearer guidance on storage practices.",
  },
};
 ...
 * Process: report p.25, with Sylvia's clarification on author/presentation bias.
 * Per-concept tradeoffs: Sylvia's account, 2026-09-10, paraphrased as farmer feedback.
 * Visual identity: table = toolkit drawing; box = sketch titled "Sun Dry Table".
 */
export const concepts = {
  ...
  process: [
    { label: "First evaluation", text: "Each concept was developed by a different team member and presented to farmers by its author." },
    { label: "Potential bias", text: "Responses seemed to reflect both the polish of the sketches and farmers' preferences for individual team members.", emphasis: true },
    { label: "Method adjustment", text: "For the second round, one person redrew all three concepts consistently, and the concept authors remained anonymous." },
  ],
  /**
   ...
   */
  options: [
    // Feedback paraphrased from Sylvia's account, 2026-09-10; not measured performance.
    { name: "Drying Table + toolkit", slug: "table", selected: false, ratio: 760 / 950,
      feedback: "Farmers felt it handled too little maize per batch. Bamboo stakes left in the ground would be inconvenient when it was not in use, and the open table offered no protection from theft or rain." },
    { name: "Drying Box", slug: "box", selected: false, ratio: 760 / 950,
      sketchNote: 'Labelled "Sun Dry Table" in the original sketch.',
      feedback: "Farmers felt the box looked limited in capacity and difficult to scale up for a larger harvest." },
    { name: "Drying Tower", slug: "tower", selected: true, ratio: 760 / 950,
      feedback: "Farmers saw more potential to process a larger batch and address all four concerns: animals, theft, rain and weevils. This was a reason to develop the concept; its performance still needed testing." },
  ],
  selected: {
    eyebrow: "Selected direction",
    note: "Selected after the second farmer evaluation.",
  },
};
...
export const finalConcept = {
  ...
  body: [
    "A passive solar drying concept: a black box collector warms air, which flows through the tower's maize trays.",
  ],
  // Report p.36 explicitly identifies shelf dimensions and a calculated maize capacity.
  specs: [
    { value: "10", label: "Trays", note: "Design specification" },
    { value: "81 × 70 × 2.5 cm", label: "Each tray (L × W × H)", note: "Design dimensions" },
    { value: "approx. 100 kg", label: "Estimated batch capacity", note: "Calculated; field test pending" },
  ],
  /**
  ...
  deliverable: {
    ...
    /** Construction guidance serves design transfer, not the PICS bag knowledge gap. */
    rationale:
      "The handbook records the parts, dimensions and assembly steps so the tower design can be shared, built locally and improved through later testing.",
...
/**
 * 09. Outcomes and evidence. Original requirements: report p.38.
 * The historical 13/18 marks are described as a design review, never pass/fail.
 * Current evidence and full-tower validation are explicitly distinguished per row.
 */
export type RequirementEvidenceKind = "design" | "calculation" | "prototype" | "pending";

export const requirementEvidenceLabels: Record<RequirementEvidenceKind, string> = {
  design: "Design expectation",
  calculation: "Calculation support",
  prototype: "Collector observation",
  pending: "Pending full-tower test",
};

/** Same 18 requirements as the original list (report p.38).
 * Evidence classification is this page's editorial assessment, not the team's ticks.
 * Collector evidence: Sylvia's recorded test account. Capacity: report p.36.
 */
export const requirementEvidence: {
  requirement: string; kind: RequirementEvidenceKind; basis: string;
}[] = [
  { requirement: "Collect or generate heat", kind: "prototype", basis: "Warm air observed at the collector outlet; tower temperatures unmeasured." },
  { requirement: "Generate airflow", kind: "prototype", basis: "A taped strip moved at the collector outlet; airflow through loaded trays untested." },
  { requirement: "Contain maize", kind: "design", basis: "Trays and an enclosure are specified; no full tower built." },
  { requirement: "Should not take up too much space in storage", kind: "design", basis: "A vertical arrangement was proposed; storage footprint not assessed with farmers." },
  { requirement: "Modular, so it can be adapted to different needs", kind: "design", basis: "Separate collector and tray components; practical adaptation untested." },
  { requirement: "Space-efficient, scaling to the quantity of grain dried", kind: "design", basis: "Stacked trays were intended to support larger batches; scaling untested." },
  { requirement: "Grain loss during drying should be minimised", kind: "pending", basis: "Needs harvest-season comparison with tarp drying." },
  { requirement: "Prevent grain from being polluted by animals", kind: "pending", basis: "An enclosure was proposed; protection not tested." },
  { requirement: "Process more than 50 kg of grain at once", kind: "calculation", basis: "Report estimates about 100 kg across ten trays; actual drying capacity untested." },
  { requirement: "Keep the grain dry throughout the drying process", kind: "pending", basis: "Rain protection and grain moisture need measurement." },
  { requirement: "Preparation time no longer than the traditional way", kind: "pending", basis: "Needs timed comparison using a complete tower." },
  { requirement: "Bagging no slower than the traditional way", kind: "pending", basis: "Needs timed loading and unloading trials." },
  { requirement: "Drying time shorter than the traditional way", kind: "pending", basis: "Needs comparable batches, weather and final moisture levels." },
  { requirement: "Structure should not deteriorate", kind: "pending", basis: "Metal was specified for durability; long-term exposure untested." },
  { requirement: "Prevent small animals eating grain during drying", kind: "pending", basis: "An enclosure was proposed; animal access untested." },
  { requirement: "Prevent grain theft", kind: "pending", basis: "A lockable door was proposed; theft resistance untested." },
  { requirement: "Weevils eradicated from the grain after drying", kind: "pending", basis: "No pest-control trial or grain temperature measurements." },
  { requirement: "Usable by one person", kind: "pending", basis: "Tray loading, unloading and handling need farmer trials." },
];
...
export const status = {
  ...
  checklist: {
    label: "Evidence behind the requirements",
    caption: "The original team review marked 13 of 18 requirements. Those marks were a design review, not test results. The evidence below distinguishes intentions, calculations and collector observations from full-tower testing.",
  },

  /** Approved decision 3: separate, clearly titled, and never in the team's voice. */
  whyOpen: {
    heading: "What still needs testing",
    text: "Collector observations and capacity calculations do not establish tower performance. Pest control, theft protection, rain protection and one-person handling all need a complete build. Comparisons with traditional drying also need a harvest-season baseline.",
  },
