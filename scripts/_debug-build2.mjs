// Debug build: only geometry near the real HALOGRIP detail cluster (buttons/TAG/labels),
// to find what (if any) handle/grip shell geometry actually belongs to the device,
// as opposed to the huge unrelated seat/mechanism scene sharing the same layers.
...
// generous region around the known detail cluster (x:-49..64, y:47..82, z:-47..-10)
const REGION = { xmin: -400, xmax: 400, ymin: -300, ymax: 400, zmin: -400, zmax: 200 };
...
for (let i = 0; i < objs.count; i++) {
  ...
  const inside = cx >= REGION.xmin && cx <= REGION.xmax && cy >= REGION.ymin && cy <= REGION.ymax && cz >= REGION.zmin && cz <= REGION.zmax;
  if (!inside) {
    skippedOutside++;
    continue;
  }
...
console.log("Added (inside region):", added, "Skipped (outside region):", skippedOutside);
